...
url = 'https://tsoua.com/wp-content/uploads/data/cdd/ARCHIVE/INFLOW/UA/'
dir_raw = 'inflow_ru_ua_raw'
start_pattern = 'Inflow'
rar_file = 'Inflow_ua-2019.rar'
...
countries_dict = {"Ужгород": "Словаччина", 
                  ...
                  "Теково": "Румунія",
                  "Молдова": "Молдова"}
dir_dict = {"Надходження всього": "вхід", "Транзит всього": "вихід", "Точки входу всього": "вхід", "Точки виходу всього": "вихід"}
col_dict = {"Надходження всього": "inflow", "Транзит всього": "transit", "Точки входу всього": "inflow", "Точки виходу всього": "transit", "Всього": "transit"}
...
def get_gas_data(file):
    ...
    title = [item for item in df_raw['station'] if pd.notna(item)][0].replace('Надходження та транзит російського газу за ', '').replace('Точки входу та виходу за ', '').replace('Точки виходу за ', '')
    ...
    if df['direction'].isnull().all():
        if 'Точки виходу за' in [item for item in df_raw['station'] if pd.notna(item)][0]:
            df['direction'] = 'вихід'
    ...
    df_total = df[df['station'].str.contains('Надходження всього|Транзит всього|Точки входу|Точки виходу|Газпром|Рос. газ|OBA|ОВА')]
    if df_total.shape[0]==0:
        df_total = df[df['station'] == 'Всього']
    df_comp = df[~df['station'].str.contains('Надходження всього|Транзит всього|Точки входу|Точки виходу|Газпром|Рос. газ|OBA|ОВА')]
    df_comp = df_comp[df_comp['station']!='Всього']
